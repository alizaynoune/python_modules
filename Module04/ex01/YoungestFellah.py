...
def youngest_fellah(dataset, year):
    ...
    if not isinstance(dataset, pd.DataFrame)\
            or not isinstance(year, int) or year <= 0:
        print('Error:\
dataset must be pandas.DataFrame and year must be positive integer')
        return None
    if not hasattr(dataset, 'Year')\
            or not hasattr(dataset, 'Sex') or not hasattr(dataset, 'Age'):
